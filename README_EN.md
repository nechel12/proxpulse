...
Fast proxy checker with a handy UI: liveness, ping, geo, anonymity,
...
the check — uncounted proxies are marked `cancelled` and never reach
the table.
...
log — actually any text file, formats are recognized automatically.
...
- The pool merges (old entries are kept); “Sync” tops it up with alive
  results, “Clear” wipes it
...
webhooks. Proxy passwords only go to the proxies themselves in auth
headers — nowhere else.

Stored locally in the browser: language, ping limit, webhooks, geo cache
(up to 2000 entries), recent errors. Clearing localStorage wipes it.
